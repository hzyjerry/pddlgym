...
NUM_OBJECTS = 7
CLEAR, PLAYER1, PLAYER2, STONE, STONE_AT_GOAL, GOAL, WALL = range(NUM_OBJECTS)

TOKEN_IMAGES = {
    PLAYER1 : plt.imread(get_asset_path('sokoban_player.png')),
    PLAYER2 : plt.imread(get_asset_path('sokoban_player2.png')),
...
def build_layout(obs, goal=None):
    ...
    for r, c in get_locations(obs, 'player-02'):
        layout[r, c] = PLAYER2
        seen_locs.add((r, c))
...
def build_layout_egocentric(obs,size=5, goal=None):
    ...
    for r, c in get_locations(obs, 'player1'):
        player1_r, player1_c = r, c
        offset_r, offset_c = r - width, c - width

    def within_view(r,c):
        return (abs(r - player1_r) <= width) and (abs(c - player1_c) <= width)
